...
`endif
                    ...
                    if ((test_pass & !test_error)) begin
                        ...
                        $write("\033[0;31mTest failed\033[0m \t%0d \t%0d\n",watchdogs_cnt,TIMEOUT);
                    end
                    watchdogs_cnt <= '0;
                `endif  // SIGNATURE_OUT
            end else begin
...
`endif // SIGNATURE_OUT
    ...
    if (watchdogs_cnt == TIMEOUT) begin
        if (test_pass == 1 && !test_error) begin
        // if (test_file == "watchdog.hex") begin
            ...
            tests_total  += 'b1;
            tests_passed += 'b0;
            $write("\033[0;31mError: TIMEOUT\t%0d  %s\033[0m\n",TIMEOUT, test_file);
            $fwrite(f_results, "%s\t\t%s\t%s\t%0d\t%0t\n", test_file, "OK" , "__FAIL",$time,$realtime);
            test_running  <= '0;
            watchdogs_cnt <= '0;
        end
    end
